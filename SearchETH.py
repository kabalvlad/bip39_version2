...
def do_work_loop(entropy_bits):
    mnemonic = create_valid_mnemonics(entropy_bits)
    private_key = mnemonic_to_private_key(mnemonic,
                                          str_derivation_path=f'{ETH_DERIVATION_PATH}/0')
    public_key = PublicKey(private_key)
    return mnemonic, public_key.address(), binascii.hexlify(private_key).decode("utf-8")
...
def check_address(addr, eth_list, mnem, pvk):
    if addr in eth_list:
        ...
        text = {mnem + ' Address: ' + addr + ' Private key: '+ pvk}
        try:
...
def generate_mnem_address_pairs():
    ...
    while True:
        ...
        k += 1
        check_address(addr, eth_list, mnem, pvk)
